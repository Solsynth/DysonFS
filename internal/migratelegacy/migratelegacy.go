...
func (m *Migrator) migrateFiles(summary *Summary) error {
	...
	for _, row := range rows {
		...
		file := database.CloudFile{ID: row.ID, Name: row.Name, Description: desc, AccountID: parseUUID(row.AccountID), PoolID: nil, ObjectID: row.ObjectID, ParentID: nil, Indexed: row.Indexed, IsFolder: row.IsFolder, IsMarkedRecycle: row.IsMarkedRecycle, ExpiredAt: row.ExpiredAt, StorageURL: row.StorageURL, StorageKey: row.StorageID, FileMeta: row.FileMeta, UserMeta: row.UserMeta, CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt}
		if row.DeletedAt != nil {
