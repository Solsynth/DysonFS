...
Legacy direct uploads create a visible file immediately after the completion
request succeeds with status `Processing`; clients should not wait for
`Completed` before displaying it. Client-assisted uploads validate the
thumbnail and persist all required derivatives synchronously, so they return
...
For client-assisted media uploads, `client_analysis` may contain positive
`width`, `height`, `duration_ms`, `rotation`, `aspect_ratio`, `sample_rate`,
and `channels`, depending on the content type. Set `want_thumbnail: true` when
the client will upload a generated JPEG thumbnail for video or another media
type. Set `want_compression: true` when the client will upload a generated
WebP image derivative. The server persists this metadata as client-provided
and validates each uploaded derivative before creating its child. Client
metadata never replaces source-size, storage, MIME, quota, or authorization
checks.
...
When `want_compression` is true, the response also includes
`compression_upload_url` and `compression_key`. Upload the generated WebP to
that URL before completion; it becomes the `system.compression.low` child.

When `want_thumbnail` is true, the response also includes
`thumbnail_upload_url` and `thumbnail_key`. Upload the generated JPEG to that
URL before the source completion request; the thumbnail is stored separately.
...
10. Finalizes media metadata. For a client-assisted upload, the server
    validates each requested derivative with `Stat`, requires a non-empty
    bounded image object, persists the validated client analysis, and creates
    the requested `system.thumbnail` and/or `system.compression.low` child.
    It does not download or probe the original source. If the client did not
    provide the requested metadata and derivative required for the declared
    media type, DysonFS downloads the committed object and recomputes the
    source metadata: SHA-256 hash plus the same EXIF / dimensions / blurhash /
    media probe analysis the proxied flow runs on the staged file. The media
    type is resolved authoritatively from the downloaded bytes (the declared
    type is trusted unless it is the generic `application/octet-stream`, in
...
When `want_thumbnail` is true, the response also includes
`thumbnail_upload_url` and `thumbnail_key`. Upload the generated JPEG to that
URL before completing the multipart session; it is a separate object from the
source parts.

When `want_compression` is true, the response also includes
`compression_upload_url` and `compression_key`; upload the generated WebP
before completing the multipart session.
